...
{- in the binomial tree Node r v ts,
   ...
   INVARIANT: the list ts has r elements (sub-trees), ordered by decreasing ranks 
r-1, r-2, ..., 1, 0.
 -}
data BinoTree a = Node Int Int a [BinoTree a]
                deriving (Show)
{- A binomial heap consisting of trees t1, ..., tn is given by BinoHeap [t1, ..., 
tn].
   INVARIANT:
...
{- extractMin h
   PRE:  h is not empty
   RETURNS: (k, h-k), where k is the minimum key of h, and h-k is h without an element of key k
 -}
