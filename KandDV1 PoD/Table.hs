...
module Table(Table, empty, insert, exists, lookup, delete, iterate, keys, values) 
where
...
{- lookup t k
   RETURNS: Just the value associated with key k in the table t, or None if the key
is absent
 -}
